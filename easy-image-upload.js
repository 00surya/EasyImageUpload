...
const EasyImageUpload = (() => {
    ...
    function createImagePreviewCard(src, imageID) {
    ...
    function imageEdit(imageElem) {
        ...
        const prevImageCard = createImagePreviewCard(imageURL, imageID)
    ...
    function insertImage() {
        ...
        if (!isBase64Image(selectedImageUrl)) {
            if (currSelectedBtn) {
                ...
                const parentContainer = currSelectedBtn.parentNode;

                const chooseImgStatus = parentContainer.querySelector('.es-im-up-status')
                if (chooseImgStatus){
                    chooseImgStatus.innerText = 'Image selected!'
                }
